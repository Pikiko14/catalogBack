...
class Utils {
     ...
     * @param {string} name
     */
    generateToken = async ({name, scopes, _id, parent }: User) => {
        const jwt = await sign({ _id, name, scopes, parent }, this.JWT_SECRET, {
          expiresIn: "1d",
        });
        return jwt;
    }
     ...
     */
    comparePassword = async (userPassword: string, loginPassword: string): Promise<boolean> => {
